...
type Area struct {
	...
	Name   string   `gorm:"size:32" json:"aname"`       //区域名字 昌平 海淀  朝阳
...
func InitDb()(*gorm.DB, error){
	...
	conn, err := gorm.Open("mysql","root:123456@tcp(127.0.0.1:3306)/ihome40?charset=utf8&parseTime=True&loc=Local")
	if err == nil {
		conn.SingularTable(true)
		conn.AutoMigrate(new(User),new(House),new(Area),new(Facility),new(HouseImage),new(OrderHouse))
		conn.DB().SetMaxIdleConns(10)
		conn.DB().SetConnMaxLifetime(100)
	}
	GlobalConn = conn
